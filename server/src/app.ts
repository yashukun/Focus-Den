...
export async function buildApp(store: StateStore, secret: string, opts: AppOptions = {}) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const user: UserRow = { id, name: trimmed, salt, hash, createdAt: Date.now(), tokenVersion: 1 };
  ...
  // Username + password only. Forgotten passwords are recovered by the admin
  // (`npm --prefix server run reset-password`), not by email.
  app.post(
    '/api/auth/login',
    { config: { rateLimit: AUTH_RATE } },
    async (req, reply) => {
      // `identifier` kept alongside `name` for back-compat with older clients.
      const body = (req.body ?? {}) as { identifier?: string; name?: string; password?: string };
      const who = (body.identifier ?? body.name ?? '').trim();
      const user = store.getUser(idFor(who));
